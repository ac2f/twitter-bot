...
class TwitterBot:
    def __init__(self, credentialsFilePath: str, usernamesFilePath: str):
        ...
        self.xpathMap["postRetweetButton"] = '//div[@data-testid="retweet"]'
        self.xpathMap["postLikeButton"] = '//div[@data-testid="like"]'
            ...
            while True:
                ...
                try:
                    driver.find_element_by_xpath(
                        self.xpathMap["usernameEntry"]).send_keys(username)
                    driver.find_element_by_xpath(
                        self.xpathMap["loginPageNext0"]).click()
                    print(
                ...
                while True:
                    try:
                        driver.find_element_by_xpath(
                            self.xpathMap["passwordEntry"]).send_keys(password)
                        driver.find_element_by_xpath(
                            self.xpathMap["loginPageNext1"]).click()
                        print(
                ...
                try:
                    driver.find_element_by_xpath(
                        self.xpathMap["wrongPassword"])
                    print(
                        f"{Fore.LIGHTMAGENTA_EX}[-] \"{username}\" için şifre yanlış!")
                except:
                    self.drivers.append({
                        "username": username,
                        "password": password,
